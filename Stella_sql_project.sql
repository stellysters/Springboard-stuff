...
SELECT name,
       ...
       monthlymaintenance
  FROM Facilities
  WHERE (membercost > 0.0) AND (monthlymaintenance * 0.20 > membercost)
...
SELECT DISTINCT CONCAT(Facilities.name, ', ', Members.surname, ', ', Members.firstname) AS "Tennis Bookings"
	FROM Facilities
	JOIN Bookings
...
SELECT CONCAT (Bookings.starttime, ', ', Members.surname, ',', Members.firstname) AS "Cost > 30",
...
/* Q9: This time, produce the same result as in Q8, but using a subquery. */
 ...
 SELECT CONCAT(member, ', ', facility, ', ', cost) AS "Cost > $30" FROM (
    SELECT CONCAT(Members.surname, ', ',  Members.firstname) as member, Facilities.name AS facility,
    CASE WHEN Members.memid = 0 THEN (Bookings.slots * Facilities.guestcost)
         ELSE (Bookings.slots * Facilities.membercost)
    END AS cost
    FROM Facilities
